...
//!
//! Trees carry interned [`Tok`] ids. `Var`/`Node.op` comparisons resolve through
//! `TokenView::str_cmp` (STRING order, exactly Python's tuple comparison -- id order would be
//! interning order, which is wrong); the leaf float test is the per-id `sort_float` property
//! (`token.parse::<f64>().ok()`, the same `float(token)` stand-in as before).
...
/// across kinds (0<1<2); within a kind, `Var` by token STRING (byte order == code-point order for
/// the ASCII grammar tokens; resolved via the view), `Num` by `total_cmp` (only 0.0/1.0 ever occur),
/// `Node` by `(len, children, op)` -- where `Vec<Key>` is lexicographic with shorter<longer on a
/// common prefix, matching Python tuple comparison of unequal-length key tuples.
fn key_cmp(a: &Key, b: &Key, view: &TokenView) -> Ordering {
    match (a, b) {
        (Key::Var(x), Key::Var(y)) => view.str_cmp(*x, *y),
        ...
        (
            ...
            .cmp(l2)
            .then_with(|| vec_key_cmp(c1, c2, view))
            .then_with(|| view.str_cmp(*o1, *o2)),
        _ => tag(a).cmp(&tag(b)),
    }
}
...
fn vec_key_cmp(a: &[Key], b: &[Key], view: &TokenView) -> Ordering {
    for (x, y) in a.iter().zip(b.iter()) {
        match key_cmp(x, y, view) {
...
fn operand_key(node: &Node, view: &TokenView) -> Key {
    match node {
        ...
        Node::Op { token, operands } => Key::Node {
            len: prefix_len(node),
            children: operands.iter().map(|o| operand_key(o, view)).collect(),
            op: *token,
        },
        // Leaf: `try (1, float(token)) except ValueError: (0, token)`. Per-id `sort_float`
        // (`token.parse::<f64>().ok()`): Rust `f64::from_str` agrees with Python `float()` on every
        // leaf in the corpus (`0`/`1` parse; `<constant>` / `x*` / `(-1)` / `np.pi` /
        // `float("inf")` all reject -> `Var`); the two known divergences (digit underscores,
        // surrounding whitespace) do not occur in the grammar's leaf tokens.
        Node::Leaf(t) => match view.sort_float(*t) {
            Some(f) => Key::Num(f),
            None => Key::Var(*t),
        },
    }
...
fn collect_positions(node: &Node, operator: Tok, path: &mut Vec<usize>, out: &mut Vec<Vec<usize>>) {
...
fn sort_commutative_node(subtree: Node, operator: Tok, view: &TokenView) -> Node {
    ...
    let keys: Vec<Key> = operands_to_sort
        .iter()
        .map(|n| operand_key(n, view))
        .collect();
    ...
    order.sort_by(|&i, &j| key_cmp(&keys[i], &keys[j], view));
...
/// monotonically right-nests; once right-nested the sort is idempotent under the total-order
/// key); the `0..=len` bound is a safety net.
pub fn sort_operands_unit(expression: &[Tok], view: &TokenView) -> Vec<Tok> {
    let mut prev = expression.to_vec();
    for _ in 0..=expression.len() {
        let next = sort_operands_once(&prev, view);
        if next == prev {
            return next;
        }
        prev = next;
...
fn sort_operands_once(expression: &[Tok], view: &TokenView) -> Vec<Tok> {
    ...
    while i >= 0 {
        ...
        if let Some((operator, arity)) = view.sort_resolve(token) {
            ...
            if view.commutative(operator) {
                ...
                stack.push(sort_commutative_node(subtree, operator, view));
                i -= 1;
                continue;
            }
