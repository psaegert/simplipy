...
(1.13.1 on the masked leg; 1 s cap, censoring stated on the panel), three
corpora — an SR training prior (n = 65,536), its raw-masked transform
(n = 65,536), and an external neutral problem set (n = 528). Scored in the
deployment space under the MDL measure, with bootstrap 95% CIs; ratio =
output/input, lower is better; "made bigger" = the fraction of rows an arm
inflated.
...
timeout on 14.7% of raw-masked rows. Paired wall-clock on the same rows:
